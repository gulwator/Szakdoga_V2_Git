    ...
    <table class="table table-striped table-bordered">
      ...
      <tbody>
        ...
        >
          ...
          <td>{{ program.iskola }}</td>
          <td>{{ program.csoport }}</td>
...
const getPrograms = async () => {
  try {
    const response = await axios.get(
      `${import.meta.env.VITE_API_BASE_URL}/programs/getPrograms`,
    );
...
const filterData = computed(() => {
  return programs.value.filter((program) => {
    ...
    const matchesProgramName = filter.value.programName
      ...
      : true;
    const matchesSchool = filter.value.school
      ? program.iskola.toLowerCase().includes(filter.value.school.toLowerCase())
      : true;
    const matchesGroup = filter.value.group
      ? program.csoport.toLowerCase().includes(filter.value.group.toLowerCase())
      : true;
